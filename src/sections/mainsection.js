               ...
               this.server.send({'request-type': 'GetCurrentScene'}).then(data=>{
                   for(let i = 0; i < data.sources.length; i++){
                       ...
                       if (data.sources[i].name === source){
                           ...
                           this.toggle = !data.sources[i].render
                           console.log("toggle is now ",this.toggle)
                       } 
                   }
               }).then(this.server.send({'request-type': 'SetSceneItemProperties',"item": source, "visible": this.toggle}))
