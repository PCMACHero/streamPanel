...
const twitchCltId = config.TwitchCredentials.twitchCltId,
      ...
      responseStr = "https://id.twitch.tv/oauth2/authorize?response_type=code&client_id=" + twitchCltId + "&redirect_uri=" + redirectUri + "&scope=channel_editor+chat:read+chat:edit+channel:moderate+viewing_activity_read+user:read:email+bits:read+clips:edit&state=" + randState;
    ...
    twitch: async (req, res) => {
        ...
        if (req.query.state === randState && req.query.code) {
            ...
            if (userIsNew.results === null) {
                ...
                console.log('what is happening here ? - user so far : ', twitchUser, ' info returned from twitch: ', jsonResponse.data.data[0]);
